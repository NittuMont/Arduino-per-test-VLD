...
class Test500VDialog(QtWidgets.QDialog):
    ...
    def _start_test(self):
        if not self.ctrl:
            ...
            return
        self.update_status("Prova 500 V in corso...", "working")
        self.result_label.setText("Test in corso...")
        self.ctrl.set_voltage(500)  # TEST_500V_VOLTAGE
        self.ctrl.set_current(2)    # ISOLATION_CURRENT_A
        self.ctrl.output_on()
        QtCore.QTimer.singleShot(1000, self._measure)
    ...
    def _measure(self):
        try:
            ...
            self.safe_power_off()
            self.update_status("Prova 500 V — errore lettura", "error")
            return
        if current <= 0.005:  # PASS_CURRENT_500V
            ...
            self.update_status("Prova 500 V superata", "ok")
        else:
            self.result_label.setText(
